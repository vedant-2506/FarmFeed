...
function setupEventListeners() {
  ...
  btnAddToInventory.addEventListener("click", async () => {
    ...
    await addToInventory(currentFertilizer.id || currentFertilizer.fertilizer_id, vendorPrice, quantity);
  });
...
function selectFertilizer(fertilizer) {
  currentFertilizer = {
    ...fertilizer,
    id: fertilizer.id || fertilizer.fertilizer_id
  };
      ...
      browseList.innerHTML = data.map(fert => `
        <div class="fertilizer-card">
          <div class="row align-items-center">
            ...
            <div class="col-md-6">
              <button class="btn btn-success" onclick='selectAndAdd({"id": ${JSON.stringify(fert.fertilizer_id || fert.id)}, "name": ${JSON.stringify(fert.name)}, "price": ${JSON.stringify(fert.price)}, "description": ${JSON.stringify(fert.description || "")}, "image_url": ${JSON.stringify(fert.image_url || "")}})'>
